...
for M_idx = M
    ...
    for SNR = SNR_db
        ...
        reset(rayChan);
        [txFaded_signal, h_ray] = step(rayChan, mod_signal);
        rx_ray = awgn(txFaded_signal, SNR, 'measured');
        
        rxEq_symbols = rx_ray ./ squeeze(h_ray);
        
        demod_symbols = fskdemod(rxEq_symbols, M_idx, freq_sep, nsamp, Fs);
...
title('BER vs SNR for FSK (Rayleigh)');
